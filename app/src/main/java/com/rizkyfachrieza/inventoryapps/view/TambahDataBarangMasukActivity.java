...
public class TambahDataBarangMasukActivity extends AppCompatActivity {
        ...
        buttonSimpan.setOnClickListener(new View.OnClickListener(){
            ...
            public void onClick(View view) {
                if(BarangSpinner.getSelectedItem().toString().equals("Pilih Kode Barang") || SupplierSpinner.getSelectedItem().toString().equals("Pilih Supplier") || tglMasuk.equals("")){
                    ...
                    nDialog = new ProgressDialog(TambahDataBarangMasukActivity.this);
                    nDialog.setMessage("Loading...");
                    nDialog.setTitle("Tambah barang Masuk");
                    nDialog.setIndeterminate(false);
                    nDialog.setCancelable(false);
                    nDialog.setCanceledOnTouchOutside(false);
                    nDialog.show();
                    sqliteHelper.addBarangMasuk(responseBarang.get(BarangSpinner.getSelectedItemPosition()-1).getId(), responseSupplier.get(SupplierSpinner.getSelectedItemPosition()-1).getId(),tglMasuk);
                    new Handler().postDelayed(new Runnable() {

                        @Override
                        public void run() {
                            Intent intent=new Intent(TambahDataBarangMasukActivity.this, DataBarangMasukActivity.class);
